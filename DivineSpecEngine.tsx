...
import { 
  ...
  ArrowRightLeft, Lock, Server, Cloud, Building2, FileText, Key
} from "lucide-react";
...
// Words that represent ROLES (users), not ENTITIES (data)
// We penalize these to avoid "model Designer {}" when we want "model Portfolio {}"
...
const generateArchitecture = (input: string) => {
  ...
  // B. ENTITY SELECTION (IMPROVED SCORING)
  let bestEntity = "item";
  let highestScore = -50; 

  // Map for generic fallbacks
  ...
  rawWords.forEach(w => {
    ...
    // Penalize explicit roles (User-centric words)
    // "Designer" -> Penalized. "Server" -> Safe. "Contractor" -> Safe.
  ...
  // Explicit Relation Name for Organization
...
`);
  ...
  // --- CORE ENTITY CONSTRUCTION ---
  // Explicit Relation Names for Organization to prevent ambiguity with User relation
  let entityFields = `
  id          String   @id @default(uuid())
  title       String
  ...
  // --- MODULE 1: COGNITIVE ---
  if (vibes.cognitive) {
    features.add("Audience Profiling");
    features.add("Adaptive Complexity");
    features.add("Metaphor Engine");
...
`);
  ...
  if (vibes.media || vibes.social) {
    features.add("File Storage (S3/R2)");
    stack.storage = "AWS S3 / Cloudflare R2";
...
`);
  ...
  // --- MODULE 4: REALTIME ---
  if (vibes.realtime) {
    features.add("WebSocket / PubSub");
    features.add("Live Presence");
...
`);
  ...
  // --- MODULE 5: SOCIAL ---
  if (vibes.social) {
    features.add("Role-Based Sharing");
    features.add("Comments System");
...
`);
  ...
  // --- MODULE 6: SECURE (AUDIT LOGS) ---
  if (vibes.secure) {
    features.add("Audit Logging");
    features.add("Compliance Tracking");
...
`);
  ...
  // --- MODULE 7: B2B/MULTI-TENANCY ---
  if (vibes.b2b) {
    features.add("Multi-Tenancy (Orgs)");
    features.add("Team Management");
    
    // Explicit relation names to avoid User/Organization conflicts
    schemaBlocks.push(`
model Organization {
  id        String   @id @default(uuid())
  name      String
...
`);
  ...
  // --- DEFAULT ROUTES ---
  routeList.unshift(
    `GET /api/${EntityPlural} (List)`,
    `POST /api/${EntityPlural} (Create)`,
    `GET /api/${EntityPlural}/:id (Detail)`,
    `PATCH /api/${EntityPlural}/:id (Update)`,
    `DELETE /api/${EntityPlural}/:id (Remove)`
  );
  ...
  return { 
    stack: {
      ...stack,
      features: Array.from(features).length > 0 ? Array.from(features) : ["Standard CRUD", "Auth"]
    },
...
const StatusLine = ({ text, delay }: { text: string; delay: number }) => (
  <motion.div 
    ...
    className="flex items-center space-x-2 text-xs font-mono text-green-400/80"
  >
...
const FeatureChip = ({ text }: { text: string }) => (
  <span className="text-[10px] bg-blue-500/10 text-blue-200 px-2 py-1 rounded border border-blue-500/20 whitespace-nowrap">
    {text}
  </span>
);

const GUMROAD_PRODUCT_ID = "REPLACE_WITH_GUMROAD_PRODUCT_ID";
    ...
    <div className="min-h-screen bg-[#050505] text-gray-100 font-sans p-4 md:p-8 flex flex-col items-center">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 text-center space-y-3">
        ...
        <p className="text-gray-500 text-sm max-w-md mx-auto">
          Generates system architectures that enforce structural harmony.
        </p>
      </motion.div>
        ...
        <div className={`relative bg-[#0A0A0A] rounded-xl border ${accessState === "ERROR" ? "border-red-500/50" : "border-white/10"} p-1 flex items-center shadow-2xl focus-within:border-blue-500/50 transition-colors`}>
          <div className="pl-4 pr-2">
            {accessState === "UNLOCKED" ? (
              ...
              <Key className={`w-5 h-5 ${accessState === "ERROR" ? "text-red-500" : "text-yellow-500"}`} />
            )}
          </div>
        ...
        <AnimatePresence mode="wait">
          {status === "COMPLETE" && spec ? (
            <motion.div 
              ...
              className="bg-black border border-blue-500/20 rounded-xl overflow-hidden shadow-2xl flex flex-col h-full"
            >
              {/* BLUEPRINT HEADER */}
              <div className="bg-white/5 border-b border-white/5 p-6 flex justify-between items-start">
                <div>
                  ...
                  <div className="flex gap-2">
                    <div className="text-[10px] font-mono text-blue-300 bg-blue-900/20 px-2 py-1 rounded border border-blue-500/20 uppercase tracking-wider">
                      {spec.type}
                    </div>
                  </div>
                </div>
                <div className="text-right hidden sm:block">
                  ...
                  <div className="text-2xl font-bold text-white font-mono">
                    {spec.warnings && spec.warnings.length > 0 ? "80%" : "100%"}
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 grid gap-8">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {spec.stack.features.map((feat: string, i: number) => (
                      <FeatureChip key={i} text={feat} />
                    ))}
                    {spec.stack.ai !== "None" && (
            ...
            <div className="h-full border-2 border-dashed border-white/5 rounded-xl flex flex-col items-center justify-center text-gray-700 font-mono space-y-4 min-h-[400px]">
               {accessState === "UNLOCKED" ? (
                  <>
                    ...
                    <div className="text-xs text-gray-800 max-w-xs text-center">
                      Enter a system description to generate a harmonized structural blueprint.
                    </div>
                  </>
               ) : (
                  <>
                    ...
                    <div className="text-xs text-gray-600 max-w-xs text-center">
                      Valid Gumroad license required to operate Divine Spec Engine.
                    </div>
                  </>
               )}
            </div>
